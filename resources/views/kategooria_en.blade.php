            ...
            <div class="w-full h-full grid grid-cols-3 gap-2">
                @foreach ($categories as $category)
                    <div class="flex flex-col w-3/3 h-full rounded-md text-center border-2 border-gray-500 text-black">
                        <div class="flex justify-center p-2 w-full "><img class="h-80 w-full rounded-t-md object-cover" src="{{ asset('storage/images') . '/' . $category->image}}" alt="{{ $category->title_en }} cover image"></div>
                        <div class="p-8  flex flex-col">
                            <div class="text-4xl pb-4"><h1>{{ $category->title_en }}</h1></div>
